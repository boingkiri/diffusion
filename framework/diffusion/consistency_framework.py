...
class CMFramework(DefaultModel):
    def __init__(self, config: DictConfig, rand_key, fs_obj: FSUtils, wandblog: WandBLog):
        ...
        def torso_loss_fn(update_params, total_states_dict, args, has_aux=False):
            ...
            if diffusion_framework['score_feedback']:
                ...
                score_feedback_ratio = jnp.floor(total_states_dict['torso_state'].step / self.step_scale).astype(int) / diffusion_framework['train']["total_step"]
        ...
        def joint_training_loss_fn(update_params, total_states_dict, args, has_aux=False):
            ...
            head_params = update_params['head_state']
            target_model = jax.lax.stop_gradient(total_states_dict['torso_state'].target_model) \
                                if diffusion_framework["sigma_sampling_joint"] != "iCT" \
                                else jax.lax.stop_gradient(torso_params) 
            ...
            if diffusion_framework["embedding_flag"]:
                rng_key, embedding_key = jax.random.split(rng_key, 2)
                embedding_flag = jax.random.bernoulli(embedding_key, shape=(y.shape[0], ))

                head_D_x = jnp.where(embedding_flag[:, None, None, None], head_D_x, jnp.zeros_like(head_D_x))
                head_t_emb = jnp.where(embedding_flag[:, None], head_t_emb, jnp.zeros_like(head_t_emb))
                head_last_x_emb = jnp.where(embedding_flag[:, None, None, None], head_last_x_emb, jnp.zeros_like(head_last_x_emb))
            ...
            if diffusion_framework['alternative_training']:
                ...
                if not self.CM_freeze:
                    ...
                    target_model_ema_decay = jnp.where(
                        ...
                        self.target_model_ema_decay_fn(jnp.floor(torso_state.step / self.step_scale).astype(int)),
                ...
                if diffusion_framework['sigma_sampling_torso'] == "CT":
                    target_model_ema_decay = self.target_model_ema_decay_fn(jnp.floor(torso_state.step / self.step_scale).astype(int))
                else:
        ...
        def sample_independent_edm_fn(params, x_cur, rng_key, gamma, t_cur, t_prev):
            ...
            # Euler step
            D_x = jnp.zeros_like(x_hat)
            t_emb = jnp.zeros(t_emb_dims, dtype=jnp.float32)
            last_x_emb = jnp.zeros(last_x_emb_dims, dtype=jnp.float32)
        ...
        self.p_sample_edm_independent = jax.pmap(sample_independent_edm_fn)
    ...
    def init_model_state(self, config: DictConfig):
        ...
        new_torso_state = TrainState.create(
            ...
            target_model=torso_params if config["framework"]["diffusion"]["sigma_sampling_joint"] != "iCT" else None, # NEW!
            tx=model_tx
        )
    ...
    def sampling_edm(self, num_image, img_size=(32, 32, 3), original_data=None, mode="edm"):
        ...
        for t_cur, t_next in pbar:
            ...
            elif mode == "edm_embedding_flag":
                latent_sample, _ = self.p_sample_edm_independent(
                    {state_name: state_content.params_ema for state_name, state_content in self.training_states.items()},
                    latent_sample, rng_key, gamma, t_cur, t_next)
    ...
    def sampling(self, num_image, img_size=(32, 32, 3), original_data=None, mode="edm"):
        ...
        if mode == "edm" or mode == "edm_embedding_flag":
